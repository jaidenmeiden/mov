...
void GLTextureV::Load(UGKS_String name)
{
	FileName = name;

	// check the file extension to see what type of texture
	if (name.find(CT_BMP_EXT))
		LoadBMP(name);
	if (name.find(CT_TGA_EXT))
		LoadTGA(name);
}

void GLTextureV::LoadFromResource(UGKS_String name)
{
	// check the file extension to see what type of texture
	if (name.find(CT_BMP_EXT))
...
void GLTextureV::LoadBMP(UGKS_String name)
{
	...
	TextureImage[0] = auxDIBImageLoad(UGKS_string2LPCWSTR(name));
...
void GLTextureV::LoadTGA(UGKS_String Name)
{
...
void GLTextureV::LoadBMPResource(UGKS_String name)
{
	// Find the bitmap in the bitmap resources
	HRSRC hrsrc = FindResource(0, UGKS_string2LPCWSTR(name), RT_BITMAP);
...
void GLTextureV::LoadTGAResource(UGKS_String name)
{
	...
	HRSRC hrsrc = FindResource(0, UGKS_string2LPCWSTR(name), L"TGA");
...
void GLTextureV::SetPathFile(UGKS_String filename){ FileName = filename; };
