...
void CSIGame::Init(void)
{
	...
	active_splash			= false;// SPLASH SCREEN activ� apr�s l'interm�de
	initialWidthW			= 600;
	initialHeightW			= 500;
...
void CSIGame::Render(void)
{
	...
	case CHAR_3D:
		...
		if (CSIG_PASSING_2_3D == AI->GetState())
			glTranslatef ( 0.0, 0.0, Camera->zoom_total_bonus);					// Camera->zoom pendant le passage � mode 3D
		else
			glTranslatef ( 0.0, 0.0, Camera->zoom_total );						// Camera->zoom utilisateur
		
		// CAMERA -> modification pendant l'interm�de (rotation de la sc�ne)
		//
		// la rotation de l'interm�de ne se fait pas directement avec un angle incr�ment�
		// pour �viter des op�rations trop nombreuses sur la matrice MODEL_VIEW, mais sur
		// un angle global apr�s la r�initialisation de la matrice par glLoadIdentity() (ci-dessus)
		glRotatef(intermede_angle_total, -0.3f, 0.55f, 1.0f);			// rotation de l'interm�de -.3 .5 1.0

		// CAMERA -> modification utilisateur (clic et d�placement de la souris)
		...
		{
			/// End of SHIPS Lights
			glEnable(SIGLB_PLAYER_LIGHT);	// �teint la lumi�re Player
			glDisable(SIGLB_SHIP_LIGHT);
			glDisable(SIGLB_SHIP2_LIGHT);
		
			/// End 3D WRAPPER
			glColor4f (1.0f, 1.0f, 1.0f, 1.0f);
			glDisable(GL_DEPTH_TEST);		// d�sactive le test de profondeur
			glEnable(GL_BLEND);				// active le m�lange de couleurs (ici transparence)
			glDisable(GL_CULL_FACE);		// on affiche les faces "cach�es" (le dos des �l�ments en mode 2D)
		}else 
			glDisable(GL_BLEND);
...
void CSIGame::InitGraphics() 
{
	...
	if (!Application.Window.FullScreen && !Application.Window.quickFullScreen)
		if (Application.Window.Width < GraphicsCard.DeskTop.Width && Application.Window.Height < GraphicsCard.DeskTop.Height) // on ne centre que si la fen�tre est + petite que le bureau
	{
	...
	// lumi�re (r�fl�chie sur les mat�riaux)
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, mat_ambient);
	
	// PASSAGE 2D � 3D -> camera path, courbe de B�zier (mouvement de cam�ra pour le passage au mode 3D)
...
bool CSIGame::Initialize (void)
{
	...
	//RT-Desk engine initializati�n. RT-Desk is a Discrete Decoupled Simulation and message management Tool
	AssignRTDESK();
...
void CSIGame::Display()
{
	...
	if (Application.Window.Width >= UGKCGC_SVGA_HOR_RES)								// logs affich�s si r�solution >= 800x600
		for(int i=0; i<Log.LogIndex; i++)							// pour toutes les entr�es de log
		{
...
void CSIGame::RunMainLoop()
{
	...
	switch (AI->GetState())
	{
		...
		case CSIG_FADING_OUT2END:
			///ACHTUNG: Aqu� deber�a incluirse el c�digo para eliminar a TODOS los objetos del sistema, no s�lo a las SS
			///Esto deber�a adem�s ser incluido como parte de la acci�n del estado LOST del juego
