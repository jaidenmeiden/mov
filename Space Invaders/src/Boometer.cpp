...
{
	Position.v[XDIM]				= orig.v[XDIM];
	Position.v[YDIM]				= orig.v[YDIM];
	CalculateAABB(h, w, CHAR_AABB_THICKNESS_DEFAULT);
...
void CBoometer::Render(float levelRatio)
{
	...
	float innerWidth = (AABB[CHAR_BBSIZE][XDIM].Value*levelRatio);
	...
	glBegin(GL_LINE_STRIP);
		...
		glVertex2f(Position.v[XDIM] + AABB[CHAR_BBSIZE][XDIM].Value, Position.v[YDIM]);//vertex 2
		glVertex2f(Position.v[XDIM] + AABB[CHAR_BBSIZE][XDIM].Value, Position.v[YDIM] + AABB[CHAR_BBSIZE][YDIM].Value); //vertex 3
		glVertex2f(Position.v[XDIM], Position.v[YDIM] + AABB[CHAR_BBSIZE][YDIM].Value); //vertex 4
	...
	if(levelRatio >0)
	{
		...
		glBegin(GL_QUADS);
			...
			glTexCoord2f (levelRatio, 0.9);	glVertex2f(Position.v[XDIM] + innerWidth, Position.v[YDIM] + AABB[CHAR_BBSIZE][YDIM].Value); //vértice 3
			glTexCoord2f (0, 0.9);			glVertex2f(Position.v[XDIM], Position.v[YDIM] + AABB[CHAR_BBSIZE][YDIM].Value); //vértice 4
		glEnd();
	}
