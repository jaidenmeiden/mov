...
void CBunker:: Init(CBNK_BUNKER_TYPE T, Vector &Pos)
{
	...
	BricksPosition.Set(Pos.v[XDIM]-(AABB[CHAR_BBSIZE][XDIM].Value*0.5f)+(CB_HLF_BRICK_WIDTH),
					   Pos.v[YDIM]+(AABB[CHAR_BBSIZE][YDIM].Value*0.5f)-(CB_HLF_BRICK_LENGTH),
					   CB_BRICK_Z_POS);
	...
	case CBNK_HOME:	
		...
		for(row=0;row<CBNK_MAX_LINES;row++){
			...
			BricksPosition.v[XDIM] = Pos.v[XDIM]-(AABB[CHAR_BBSIZE][XDIM].Value*0.5f)+(CB_BRICK_WIDTH*0.5f);
			BricksPosition.v[YDIM] -= CB_BRICK_LENGTH;
			delta--;
		}
		//Update BB size values
		AABB[CHAR_BBSIZE][XDIM].Value = CB_BRICK_WIDTH*CBNK_MAX_COLUMNS;
		AABB[CHAR_BBSIZE][YDIM].Value = CB_BRICK_LENGTH*CBNK_MAX_LINES;
		AABB[CHAR_BBSIZE][ZDIM].Value = CB_BRICK_HEIGHT;
...
void CBunker::Collided (CCharacter *CollidedChar)
{
	...
	col = ((CollidedChar->Position.v[XDIM] - (Position.v[XDIM] - (AABB[CHAR_BBSIZE][XDIM].Value * 0.5f))) / CB_BRICK_WIDTH);
	row = ((CollidedChar->Position.v[YDIM] - (Position.v[YDIM] - (AABB[CHAR_BBSIZE][YDIM].Value * 0.5f))) / CB_BRICK_LENGTH);
