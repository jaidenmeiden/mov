...
void CCircleShip::Init()
{
	...
	AABB[CHAR_BBSIZE][XDIM].Value = CCS_XSIZE3D;
	AABB[CHAR_BBSIZE][YDIM].Value = CCS_YSIZE3D;
	AABB[CHAR_BBSIZE][ZDIM].Value = CCS_ZSIZE;
...
void CCircleShip::ChangeRenderMode(CHAR_RENDER_MODE Mode)
{
	...
	case CHAR_NO_RENDER:			//No render for this character: camera, collision objects,...
		break;
	case CHAR_2D:
		CalculateAABB(CCS_XSIZE2D, CCS_YSIZE2D, CCS_ZSIZE);
		break;
	case CHAR_LINES3D:
		break;
	case CHAR_3D:
...
void CCircleShip::Update(void)
{
	...
	if(Alive())
	{
		...
		if (Timer[CCS_UPD_PERIOD].IsSounding())
		{
			...
			if(msUpdCShip > SIGLBD_MIN_UPDATETIME_OBJECTS) msUpdCShip = SIGLBD_MIN_UPDATETIME_OBJECTS;
